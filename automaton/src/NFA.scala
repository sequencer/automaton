...
/**
  ...
  *
  * */
case class eNFA[State, Symbol](initialState: State, transition: Map[(State, Option[Symbol]), Set[State]], finalStates: Set[State]) {
  lazy val alphabet: Set[Symbol] = transition.keys.map(_._2).filter(_.isDefined).map(_.get).toSet
  lazy val states: Set[State] = Set(initialState) ++ transition.keys.map(_._1).toSet ++ transition.values.flatten.toSet ++ finalStates
...
case class NFA[State, Symbol](initialState: Set[State], transition: Map[(Set[State], Symbol), Set[Set[State]]], finalStates: Set[Set[State]]) {
  lazy val (singleEdge: Map[(Set[State], Symbol), Set[Set[State]]], multiEdge: Map[(Set[State], Symbol), Set[Set[State]]]) = transition.partition(_._2.size == 1)

  private lazy val cleanMultiEdge: NFA[State, Symbol] = {
    if (multiEdge.isEmpty) this
    else
      new NFA[State, Symbol](initialState = initialState, transition = {
      ...
      }.toMap, finalStates = finalStates).cleanMultiEdge
  }
